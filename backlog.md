...
- [ ] **B47 — Hide dependent settings rows when their parent feature is off (desktop).** Web V4 hides the "Hide-until (Defer)" toggle when `dueEnabled` is off, and hides the "Tag Colours" section when `tagsEnabled` is off — they have no meaning without their parent. Desktop's settings show both toggles regardless. Apply the same gating in `src/app.js` `applySettings()` and the equivalent settings DOM IDs.
