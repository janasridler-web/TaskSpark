...
### Bugs / cleanup
- [ ] **U23 — Update-banner wording is web-flavoured under the wrapped desktop.** When `TASKSPARK_USE_WEB=1` the banner says "TaskSpark has updated — refresh to get the latest" and the button calls `location.reload()`, which is correct for the web's service-worker swap but misleading on desktop (you actually need to *quit* to apply the new `.exe`). `autoInstallOnAppQuit = true` is already on, so the update lands on next quit regardless — but the banner copy should switch to something like "v4.X.X downloaded — close TaskSpark to install" with an "Install now" button that calls `api.installUpdate()`. Branch on `window.desktopAPI` inside `showUpdateBanner`.
