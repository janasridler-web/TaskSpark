...
- [ ] **U24 — Mouse-wheel scroll doesn't work in some regions of the wrapped app.** Trackpad two-finger scroll works fine; discrete mouse-wheel events don't reach certain scroll containers. Almost certainly a CSS `overflow:hidden` rule on a parent that should be `overflow:auto`, or a `pointer-events:none` somewhere catching wheel events first. Affected regions: TBD (Jana to specify when next testing — guess is kanban columns, calendar grid, or settings modal). Probably present on the pure web companion in any Mac browser too, not Mac-app specific.
...
- [ ] **B54 — Per-day mood data not syncing across devices.** User logged mood on Mac, doesn't appear on Windows the same day. Code path looks right on paper: `saveTodayMood` → `moodAppend` writes to the hidden "Mood History" sheet tab; `syncTodayMoodFromCloud` runs from `runPostInitWireup` on the second device and pulls it back. To diagnose: (1) open the Google Sheet's "Mood History" tab — is there a row for the date when mood was logged? If no, save side is failing silently (the catch in `saveMoodHistory` swallows; check DevTools console on the device doing the logging). If yes, sync side is failing on the second device (check DevTools console for "Failed to sync today's mood"). Once we know which, the fix is targeted.
