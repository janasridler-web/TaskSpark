...
function filterTasks() {
  ...
  return tasks.filter(task => {
    ...
    if (v === 'status-not-started')   return !task.completed && (task.status || 'not-started') === 'not-started';
    if (v === 'status-in-progress')   return !task.completed && task.status === 'in-progress';
    if (v === 'status-blocked')       return !task.completed && task.status === 'blocked';
    if (v === 'status-on-hold')       return !task.completed && task.status === 'on-hold';
    if (v.startsWith('tag:'))         return !task.completed && (task.tags||[]).includes(v.slice(4));
    return false;
  });
}
...
function calcStreak() {
  ...
  while (true) {
    ...
    // Skip vacation days
    if (settings.vacationMode && settings.vacationReturn && ds >= todayStr()) {
      check.setDate(check.getDate()-1); continue;
    }
    if (dates.has(ds)) {
