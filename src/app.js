...
api.onOauthCode(async ({ code }) => {
  ...
  try {
    ...
    if (tokens.access_token) {
      ...
      try {
        await ensureToken();
        const driveWs = await api.driveWorkspacesLoad({ accessToken, configSheetId: null });
        if (driveWs && driveWs.data && driveWs.data.workspaces && driveWs.data.workspaces.length) {
          // Found existing workspaces — load them
          configSheetId = driveWs.id;
          ...
          // No existing config — this is a fresh setup, show workspace naming modal
...
function taskCardHTML(task) {
  ...
  // Timer button
  const timerBtn = (!task.completed && settings.timerEnabled !== false) ? `
  ...
  <div class="${cardClass}" id="task-card-${task.id}">
    ...
    <div class="task-body">
      ...
      <div class="task-meta">
        ...
        ${dueBadge}${tagBadges}${timeBadge}${liveTimeBadge}${budgetBadge}${renderAttachmentBadges(task)}
      </div>
      ${completionDetail}
...
function renderCalendarView() {
  ...
  if (calViewType === 'month') {
    container.innerHTML = toolbar + renderMonthView(calDate, today, activeTasks, completedTasks);
  } else {
    container.innerHTML = toolbar + renderWeekView(calDate, today, activeTasks, completedTasks);
  }
}
...
function getItemsForDate(dateStr, taskList, eventList, completedList = []) {
  ...
  eventList.forEach(e => {
    ...
    if (dateStr >= startDate && dateStr <= endDate) {
      ...
      chips.push({ type:'event', label, id: e.id, eventType: isHolidayEvent ? 'holiday' : '' });
    }
  });
  // Then tasks
  taskList.forEach(t => {
...
function renderMonthView(date, today, activeTasks, completedTasks = []) {
  ...
  for (let i = firstDay-1; i >= 0; i--) {
    cells += renderMonthCell(calFmtDate(new Date(year, month-1, prevDays-i)), prevDays-i, true, today, activeTasks, completedTasks);
    dayCount++;
  }
  for (let d = 1; d <= daysInMonth; d++) {
    cells += renderMonthCell(calFmtDate(new Date(year, month, d)), d, false, today, activeTasks, completedTasks);
    dayCount++;
  }
  let next = 1;
  while (dayCount % 7 !== 0) {
    cells += renderMonthCell(calFmtDate(new Date(year, month+1, next)), next, true, today, activeTasks, completedTasks);
    dayCount++; next++;
  }
  return `<div class="cal-month-wrap"><div class="cal-month-header">${dowHeaders}</div><div class="cal-month-body">${cells}</div></div>`;
}

function renderMonthCell(dateStr, dayNum, otherMonth, today, activeTasks, completedTasks = []) {
  const isToday = dateStr === today;
  const isPast = dateStr < today && !isToday;
  const items = getItemsForDate(dateStr, activeTasks, calEvents, completedTasks);
...
function renderWeekView(date, today, activeTasks, completedTasks = []) {
  ...
  const allDayItems = weekDates.map(d => {
    ...
    const items = getItemsForDate(ds, activeTasks, calEvents.filter(e => e.allDay), completedTasks);
